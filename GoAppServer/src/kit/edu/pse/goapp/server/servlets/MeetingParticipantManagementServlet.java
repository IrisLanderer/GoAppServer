...
public class MeetingParticipantManagementServlet extends HttpServlet {
        ...
        try {
            ...
            try {
                ...
                throw new CustomServerException(
                        "The ParticipantID from the JSON string isn't correct!",
                        HttpServletResponse.SC_BAD_REQUEST);

            }
            Participant participantForMeetingId = createParticipantWithDao(
                    dao.getParticipantId());
            User user = createUserWithDao(userId);
            Meeting meetingCheckParticipant = createMeetingWithDao(
                    participantForMeetingId.getMeetingId());
            meetingCheckParticipant.isParticipant(user);
            if (dao != null) {
                Participant participant = dao.getParticipantByID();
                response.getWriter().write(new ObjectConverter<Participant>()
                        .serialize(participant, Participant.class));
            }
    ...
    protected void doPost(HttpServletRequest request,
        ...
        try {
            ...
            ParticipantDAO dao = new ParticipantDaoConverter()
                    .parse(jsonString);
            dao.setConfirmation(MeetingConfirmation.PENDING);
            Meeting meetingCheckingForAuthorization = createMeetingWithDao(
                    dao.getMeetingId());
            User user = createUserWithDao(userId);
            ...
            response.getWriter().write(new ObjectConverter<Meeting>()
                    .serialize(meeting, Meeting.class));
        } catch (CustomServerException e) {
            response.setStatus(e.getStatusCode());
            response.getWriter().write(e.toString());
        }
     ...
     *             CustomServerException
     */
    private User createUserWithDao(int userId)
     ...
     *             CustomServerException
     */
    private Meeting createMeetingWithDao(int meetingId)
            throws IOException, CustomServerException {
     ...
     *             CustomServerException
     */
    private Participant createParticipantWithDao(int participantId)
            throws IOException, CustomServerException {
        ParticipantDAO participantDAO = new ParticipantDaoImpl();
        participantDAO.setParticipantId(participantId);
        Participant participantForMeetingId = participantDAO
                .getParticipantByID();
     ...
     *             CustomServerException
     */
    private void checkIfUserIsParticipantAndCreator(int userId,
            int participantId) throws IOException, CustomServerException {
        Participant participantForMeetingId = createParticipantWithDao(
                participantId);
        User user = createUserWithDao(userId);
        Meeting meetingCheckParticipant = createMeetingWithDao(
