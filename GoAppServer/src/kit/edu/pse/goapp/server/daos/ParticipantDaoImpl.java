...
public class ParticipantDaoImpl implements ParticipantDAO {
     ...
     */
    protected void addParticipant(DatabaseConnection conn)
            throws IOException, CustomServerException {
        if (meetingId <= 0) {
            throw new CustomServerException(
                    "A participant must have an MeetingID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (userId <= 0) {
            throw new CustomServerException(
                    "A participant must have an UserID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        try {
            String query = MessageFormat.format(
                    "INSERT INTO participants (users_id, meetings_id, confirmation) VALUES"
                            + " (''{0}'', ''{1}'', ''{2}'')",
                    userId, meetingId, confirmation);
            participantId = conn.insert(query);
            if (participantId <= 0) {
                throw new CustomServerException(
                        "The ParticipantID wasn't assigned to a value!",
     ...
     *             CustomServerException
     */
    protected void deleteParticipant(DatabaseConnection connection)
            throws IOException, CustomServerException {
        if (participantId <= 0) {
            throw new CustomServerException("A participant must have an ID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        try {
            String query = MessageFormat.format(
                    "DELETE FROM participants WHERE participants_id = ''{0}''",
                    participantId);
    ...
    protected List<Participant> getAllParticipants(
            DatabaseConnection connection) throws Exception {
        if (meetingId <= 0) {
            throw new CustomServerException(
                    "The MeetingId is necessary to get all participants!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        List<Participant> participants = new ArrayList<>();
        try {
            String query = MessageFormat.format(
                    "SELECT participants.participants_id FROM participants  "
                            + " WHERE participants.meetings_id = ''{0}''",
                    meetingId);
     ...
     *             Exception
     */
    protected Participant getParticipantByID(DatabaseConnection connection)
            throws Exception {
        if (participantId <= 0) {
            throw new CustomServerException("A participant must have an ID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        ...
        try {
            String query = MessageFormat
                    .format("SELECT participants_id, users_id, meetings_id, confirmation "
                            + "FROM participants "
                            + "WHERE participants_id = ''{0}''", participantId);
        ...
        Participant participant = new Participant(participantId, meetingId,
                user, confirmation);
        connection.close();
        return participant;
    }
     ...
     *             CustomServerException
     */
    protected void updateParticipant(DatabaseConnection connection)
            throws IOException, CustomServerException {
        if (participantId <= 0) {
            throw new CustomServerException(
                    "A participant must have a ParticipantID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (userId <= 0) {
            throw new CustomServerException(
        ...
        if (confirmation == null) {
            throw new CustomServerException(
                    "A participant must give a cofirmation status of a meeting!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        try {
            String query = MessageFormat.format(
                    "UPDATE participants SET confirmation = ''{0}''"
    ...
    private final class ParticipantSqlSelectionHandler
        ...
        public void handleResultSet(ResultSet resultSet)
            ...
            while (resultSet.next()) {
                ...
                confirmation = MeetingConfirmation
                        .valueOf(resultSet.getString(4).toUpperCase());
            }
            if (resultEmpty) {
                throw new CustomServerException(
                        "The selected resultset from the database is empty",
    ...
    private final class ParticipantsSqlSelectionHandler
            implements SqlSelectHandler {
        @Override
        public void handleResultSet(ResultSet resultSet)
                throws SQLException, CustomServerException {
