...
public class MeetingDaoImpl implements MeetingDAO {
     ...
     */
    protected void addMeeting(DatabaseConnection connection)
            throws IOException, CustomServerException {
        if (name == null || name.equals("")) {
            throw new CustomServerException("A new meeting must have a name!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (placeX <= 0) {
            throw new CustomServerException(
                    "A new meeting must have a x-coordinate!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (placeY <= 0) {
            throw new CustomServerException(
                    "A new meeting must have a y-coordinate!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (placeZ < 0) {
            throw new CustomServerException(
                    "A new meeting must have a z-coordinate!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (timestamp <= 0) {
            throw new CustomServerException(
                    "A new meeting must have a timestamp!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (duration <= 0) {
            throw new CustomServerException(
                    "A new meeting must have a duration!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if ((type == null) || !(type.equals("Tour") || type.equals("Event"))) {
            throw new CustomServerException("A new meeting must have a Type!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (userId <= 0) {
            throw new CustomServerException(
                    "A new meeting must have a creator!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        creatorId = 0;
        try {
            String query = MessageFormat.format(
                    ...
                    name, placeX, placeY, placeZ, Long.toString(timestamp),
                    duration, type, creatorId);
            meetingId = connection.insert(query);
            if (meetingId <= 0) {
                throw new CustomServerException(
                        "The MeetingID wasn't assigned to a value!",
            ...
            String updateQuery = MessageFormat.format(
                    "UPDATE meetings SET creator_id = ''{0}''  WHERE meetings_id = ''{1}''",
                    creatorId, meetingId);
     ...
     *             Exception
     */
    protected List<Meeting> getAllMeetings(DatabaseConnection connection)
            throws Exception {
        List<Meeting> meetings = new ArrayList<>();
        try {
            String query = MessageFormat
                    .format("SELECT meetings.meetings_id FROM meetings join participants "
     ...
     *             CustomServerException
     */
    protected void updateMeeting(DatabaseConnection connection)
            throws IOException, CustomServerException {
        if (meetingId <= 0) {
            throw new CustomServerException("A meeting must have an ID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        }
        if (name == null || name.equals("")) {
            throw new CustomServerException("A new meeting must have a name!",
        ...
        try {
            String query = MessageFormat.format(
                    "UPDATE meetings "
                            + "SET name = ''{0}'', place_x = ''{1}'', place_y = ''{2}'', place_z = ''{3}'',"
                            + " timestamp = ''{4}'', duration = ''{5}'', type = ''{6}'', creator_id = ''{7}''"
                            + " WHERE meetings_id = ''{8}''",
                    name, placeX, placeY, placeZ, timestamp, duration, type,
                    creatorId, meetingId);
     ...
     *             CustomServerException
     */
    protected void deleteMeeting(DatabaseConnection connection)
            throws IOException, CustomServerException {
        if (meetingId <= 0) {
            throw new CustomServerException("A meeting must have an ID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        ...
        try {
            String queryMeeting = MessageFormat.format(
                    "DELETE FROM meetings WHERE meetings_id = ''{0}''",
                    meetingId);
            String queryParticipants = MessageFormat.format(
                    "DELETE FROM participants WHERE meetings_id = ''{0}''",
                    meetingId);
     ...
     *             Excwption
     */
    protected Meeting getMeetingByID(DatabaseConnection connection)
            throws Exception {
        if (meetingId <= 0) {
            throw new CustomServerException("A meeting must have an ID!",
                    HttpServletResponse.SC_BAD_REQUEST);
        ...
        try {
            String query = MessageFormat.format(
                    "SELECT * FROM meetings WHERE meetings_id = ''{0}''",
                    meetingId);
        ...
        if (type.equals("Event")) {
            Event event = new Event(meetingId, name, gps, timestamp, duration,
                    creator);
            event.setParticipants(participants);
            connection.close();
            return event;
        } else {
            Tour tour = new Tour(meetingId, name, gps, timestamp, duration,
                    creator);
    ...
    private final class MeetingsSqlSelectionHandler
